...
/**
 * Resolves [OpenWakeWordDetector.HeadConfig]s for the discovered wake set: bundled models for built-in
 * words (jarvis, alexa) and ONNX assets shipped inside plugin APKs ([WakeContract.META_MODEL]).
 */
object OwwHeadResolver {

    fun resolve(context: Context, targets: List<WakeTarget>): List<OpenWakeWordDetector.HeadConfig> = buildList {
        for (target in targets) {
            ...
            add(OpenWakeWordDetector.HeadConfig(word.id, bytes, threshold))
        }
    }
