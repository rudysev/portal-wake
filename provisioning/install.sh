...
start_service() {
  ...
  # Then wait until a wake detector is actually listening before telling the user to
  # speak — the model + grammar take a few seconds to load, and a "hey jarvis" said
  # before that can be missed. The service logs "wake detector ready (...)" once it's
  # listening; count existing lines first so a reinstall (debug.txt persists) waits
  # for a NEW one rather than matching a stale line.
  local log="/sdcard/Android/data/$PKG/files/debug.txt"
  local base; base=$(a shell "grep -c 'wake detector ready' $log 2>/dev/null" | tr -dc '0-9'); base=${base:-0}
  a shell "am broadcast -a $START_ACTION -n $RECEIVER -f 0x20" >/dev/null 2>&1
  local i=0
  while [ $i -lt 30 ]; do
    local n; n=$(a shell "grep -c 'wake detector ready' $log 2>/dev/null" | tr -dc '0-9'); n=${n:-0}
