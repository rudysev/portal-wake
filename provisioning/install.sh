...
start_service() {
  ...
  # Then wait until the detector is actually listening before telling the user to
  # speak — the ONNX models take a few seconds to load, and a "hey jarvis" said before
  # that can be missed. The service logs "wake detector ready" once it's listening;
  # count existing lines first so a reinstall (debug.txt persists) waits for a NEW
  # one rather than matching a stale line.
